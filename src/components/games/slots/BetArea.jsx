...
export default React.forwardRef((props, ref) => {
  ...
  function placeBet(fromEffect) {
    ...
    const betAmount = parseFloat(betInput.current.value);
    socket.emit("slots:play", { betAmount, fastSpin });
    if (!fromEffect) {
      if (betAmount > 0.01)
        setTimeout(() => {
          props.setSpinAmount(spinAmount);
        }, 500);
    }
  }
  ...
  useEffect(() => {
    ...
    if (
      props.spinAmount > 0 &&
      userBalance >= parseFloat(betInput.current.value) * 100
    ) {
      setTimeout(() => {
        placeBet(true);
      }, 500);
    } else {
      props.setSpinAmount(0);
    }
  }, [props.spinAmount]);
    ...
    <div className={"slots__bet-area "}>
      ...
      <button
        ...
        onClick={() => {
          ...
          */
          if (props.spinAmount == 0) placeBet();
          else props.setSpinAmount(0);
        }}
      >
        {
          <React.Fragment>
            {props.spinAmount > 1 ? "CANCEL AUTO-SPIN" : "SPIN"}
          </React.Fragment>
        }
      </button>

      {props.spinAmount > 1 ? (
        <div class="spin-amo">Remaining Spins: #{props.spinAmount}</div>
      ) : (
        ""
      )}
      ...
      <div class="button-area">
        ...
        <div
          className={`slots_button ${!fastSpin && "button-off"}`}
          onClick={(e) => {
            setFastSpin(!fastSpin);
          }}
        >
          <FontAwesomeIcon icon="bolt" className="" />
          Fast Spin
        </div>
      </div>
    </div>
